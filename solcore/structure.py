...
class Layer:
    """Class that stores the information about layers of materials, such as thickness
    and composition.
    ...
    def __init__(self, width, material, role=None, geometry=None, **kwargs):
        ...
        :param width: Width of the layer, in SI units. :param material: Solcore material
        :param role: Role of the layer. :param kwargs: Any other keyword parameter which
        will become part of the layer attributes
        """
        self.width = width
    ...
    def __str__(self):
        """Representation of the Layer object :return: A string with a summary of the
        layer properties."""
...
class Junction(list):
    """Class that groups together the layers that make a junction.

    Esentially, it is just a list with attributes you can update.
    """
...
class TunnelJunction(Junction):
    """Class that contains the minimum definitions for a tunnel junction, ie.

    a series resistance in our case.
    """
...
def InLineComposition(layer):
    """Hack to use the Adachi-alfa calculator, provinding the composition as a single
    string.

    :param layer: A layer as defined in the Device structures of the PDD solver :return:
    A mterial string
    """
...
def SolcoreMaterialToStr(material_input):
    """Translate a solcore material into something than can be easily stored in a file
    and read.

    :param material_input: A solcore material :return: A dictionary with the name,
    consituents and composition of the material
    """
...
def ToSolcoreMaterial(comp, T, execute=False, **kwargs):
    """It provides a solcore material out of its string composition. The output can be a
    string with the command or a solcore material itself.

    :param comp: A Solcore material as a string :param T: The temperature :param
...
def ToStructure(device):
    """Translate the device object (as used by the PDD solver) into a list of solcore
    Structure that can be used in other plugings. Only translate the composition, for
    now. It works only on non-nested device structures (QW, for example, but not in
    devices with QW)

    :param device: A solcore device structure as used by the PDD solver :return: A
    Solcore Structure
    """
