...
class DBMaterial:
            ...
            if (data["type"].split())[0] == "tabulated":
                ...
                if (data["type"].split())[1] == "n":
                    ...
                    self.refractiveIndex = RefractiveIndexData.SetupRefractiveIndex(
                        formula=-1, wavelengths=wavelengths, values=n
                    )
                elif (data["type"].split())[1] == "k":

                    self.extinctionCoefficient = ExtinctionCoefficientData.SetupExtinctionCoefficient(
                        wavelengths, n
                    )
    ...
    def to_txt(self, output):
        ...
        if self.has_refractive():
            ...
            for i in range(len(refr)):
                output_f.write(
                    " ".join(list(map(str, [refr[i][0] * 1e-6, refr[i][1]]))) + "\n"
                )
            output_f.close()
        ...
        if self.has_extinction():
            ...
            for i in range(len(ext)):
                output_f.write(
                    " ".join(list(map(str, [ext[i][0] * 1e-6, ext[i][1]]))) + "\n"
                )
            output_f.close()
...
class RefractiveIndexData:
    ...
    def SetupRefractiveIndex(formula, **kwargs):
        ...
        :param kwargs: :return: :raise Exception:
        """
        if formula >= 0:
...
class FormulaRefractiveIndexData:
    ...
    def __init__(self, formula, rangeMin, rangeMax, coefficients, interpolation_points):
        ...
        :param rangeMin: :param rangeMax: :param coefficients:
        """
    ...
    def get_complete_refractive(self):
        ...
        extlist = [
            [wavelength[i], self.get_refractiveindex(wavelength[i] * 1000)]
    ...
    def get_refractiveindex(self, wavelength):
        ...
        if self.rangeMin <= wavelength <= self.rangeMax:
            ...
            raise Exception(
                "Wavelength {} is out of bounds. Correct range(um): ({}, {})".format(
                    wavelength, self.rangeMin, self.rangeMax
                )
            )
...
class TabulatedRefractiveIndexData:
    ...
    def get_refractiveindex(self, wavelength):
        ...
        if self.rangeMin == self.rangeMax and self.rangeMin == wavelength:
            ...
            self.rangeMin <= wavelength <= self.rangeMax
            and self.rangeMin != self.rangeMax
        ):
