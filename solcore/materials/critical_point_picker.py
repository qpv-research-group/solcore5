...
class CPPicker:
    """This class can be used to pick the critical points out of a collection of n and k
    curves for a range of compositions in ternary alloys. Later, this critical points
    can be used later to calculate the n and k data at any other intermediate
    composition.
    ...
    def gather_files(self):
        ...
        for file_path in sorted(candidate_files):
            ...
            print(
                "File '{}': material fraction guess = {} {}".format(
                    file_name,
                    fraction_guess,
    ...
    def plot_a_curve(self, curve_id, background=False):
        ...
        if curve_id == self.fraction_order[self.current_curve] and not background:
            ...
            self.current_curve != 0
            and curve_id == self.fraction_order[self.current_curve - 1]
        ):
            colour = "black"
            picker = None
            width = 1
        ...
        plt.plot(
            data[0] * 1e9, data[1], "-", color=colour, picker=picker, linewidth=width
        )
    ...
    def plot_all(self, keep_limits=True):
        ...
        for critical_point_sublist in self.critical_points:
            x, y = (
                ...
                [point[1] for point in critical_point_sublist],
            )
    ...
    def onpick(self, event):
        ...
        if self.current_curve < self.n_curves:
            if len(self.critical_points[-1]) < self.n_curves:
                print(
                    "critical for fraction {} at: x,y=({},{})".format(
                        self.fraction_order[self.current_curve], x, y
                    )
                )
    ...
    def save(self):
        ...
        for section in self.fraction_order:
            ...
            self.result_config.set(
                section, "file", self.fractions_dict[section]["file_name"]
            )

        for critical_point_number, critical_points_subset in enumerate(
            self.critical_points[:-1]
        ):
            for fraction_number, fraction in enumerate(self.fraction_order):
                c = critical_points_subset[fraction_number]
                self.result_config.set(
                    fraction,
                    str(critical_point_number + 1),
                    "{} {}".format(c[0] * 1e-9, c[1]),
                )
