...
import solcore
from solcore.absorption_calculator.nk_db import nkdb_load_k, nkdb_load_n
from solcore.absorption_calculator.sopra_db import compounds_info, sopra_database
from solcore.constants import c
from solcore.constants import electron_mass as m0
from solcore.constants import h, kb, pi, q, vacuum_permittivity
...
class MaterialSystem(SourceManagedClass, metaclass=Singleton):
    ...
    def material(self, name, sopra=False, nk_db=False):
        ...
        that contains its properties, assuming that the material does not exists in the
        database, yet.
        ...
        class SpecificMaterial(BaseMaterial):
            ...
            def __getattr__(self, attrname):  # only used for unknown attributes
                ...
                raise AttributeError(
                    'Parameter "{}" not available for this SOPRA material "{}".'.format(
                        attrname, self.material_string
                    )
                )
            ...
            def n_interpolated(self, x):
                ...
                try:
                    ...
                    print(
                        'Material "{}" does not have n-data defined. Returning "ones"'.format(
                ...
                if len(self.composition) == 0:
                    ...
                    wl, y, trash = self.data.load_composition(
                        Lambda=x * 1e9,
            ...
            def k_interpolated(self, x):
                ...
                try:
                    ...
                    print(
                        'Material "{}" does not have k-data defined. Returning "zeroes"'.format(
                            self.material_string
                        )
                    )
                ...
                if len(self.composition) == 0:
                    ...
                    wl, trash, y = self.data.load_composition(
                        Lambda=x * 1e9,
                        **{self.composition[0]: self.main_fraction * 100}
                    )
    ...
    def nk_material(self, name):
        ...
        class SpecificMaterial(BaseMaterial):
            ...
            def __getattr__(self, attrname):  # only used for unknown attributes
                ...
                raise AttributeError(
                    'Parameter "{}" not available for this refractiveindex.info material "{}".'.format(
                        attrname, self.material_string
                    )
                )
            ...
            def load_n_data(self):
                ...
                self.n_data = np.vstack(
                    (wl * 1e-6, n)
                )  # wavelengths in DB are in microns
...
class BaseMaterial:
    ...
    def __getattr__(self, attrname):  # only used for unknown attributes.
        ...
        if attrname == "electron_mobility":
            try:
                ...
                return calculate_mobility(
                    self.material_string, False, self.Nd, self.main_fraction
                )
        if attrname == "hole_mobility":
            try:
                ...
                return calculate_mobility(
                    self.material_string, True, self.Na, self.main_fraction
                )
        if attrname == "Nc":
            ...
            return (
                2 * (2 * pi * self.eff_mass_electron * m0 * kb * self.T / h ** 2) ** 1.5
            )
        if attrname == "Nv":
    ...
    def load_n_data(self):
        if len(self.composition) == 0:
            ...
            self.n_data, self.n_critical_points = critical_point_interpolate.load_data_from_directory(
                self.n_path
            )
    ...
    def load_k_data(self):
        if len(self.composition) == 0:
            ...
            self.k_data, self.k_critical_points = critical_point_interpolate.load_data_from_directory(
                self.k_path
            )
    ...
    def n_interpolated(self, x):
        ...
        if len(self.composition) == 0:
            ...
            x, y, self.critical_points_n = critical_point_interpolate.critical_point_interpolate(
                self.n_data, self.n_critical_points, self.main_fraction, x
            )
    ...
    def k_interpolated(self, x):
        ...
        try:
            ...
            print(
                'Material "{}" does not have k-data defined. Returning "zeros"'.format(
                    self.material_string
                )
            )
            return np.zeros_like(x)
        ...
        if len(self.composition) == 0:
            ...
            x, y, self.critical_points_k = critical_point_interpolate.critical_point_interpolate(
                self.k_data, self.k_critical_points, self.main_fraction, x
            )
        return y
    ...
    def latex_string(self):
        ...
        for element in self.composition:
            if self.__dict__[element] != 0:
                s = s.replace(element, "{}_{{{:.3f}}}").format(
    ...
    def html_string(self):
        ...
        for element in self.composition:
            if self.__dict__[element] != 0:
                s = s.replace(element, "{}<sub>{:.3f}</sub>").format(
                    element, self.__dict__[element]
                )
            else:
                s = s.replace(element, "")
    ...
    def __repr__(self):
        parameters = [
            "{}={}".format(key, getattr(self, key)) for key in self.key_parameters
        ]
