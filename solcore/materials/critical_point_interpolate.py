"""This module interpolates the n and k data of an alloy based on the known values at
certain specific compositions.

The way it works is by interpolating in a smart way certain critical points (usually the
Adachi critical points) and then filling the gaps in between.
"""
import os
...
def critical_point_interpolate(data, critical_points, target_fraction, grid):
    result_critical_points, (
        lower_fract,
        higher_fract,
    ), fraction_along = interpolate_critical_points(critical_points, target_fraction)
