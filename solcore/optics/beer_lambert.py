...
def solve_beer_lambert(solar_cell, options):
    ...
    using the Beer-Lambert law. Reflection is not really calculated and needs to be
    provided externally, otherwise it is assumed to be zero.

    :param solar_cell: :param options: :return:
    """
    ...
    for j, layer_object in enumerate(solar_cell):
        ...
        # coeficient and the layer width.
            ...
            if kind == "2D":
                ...
                # Otherwise, we try to treat is as a DB junction from the optical
                # point of view
                else:
...
def calculate_absorption_beer_lambert(widths, alphas, fraction):
    ...
    # At any given position, the absorption per unit length is alpha * exp(-alpha*z)
    # but we have to remove all light
    # absorbed above it:
    OD = [np.zeros(N)]
    for i in range(len(widths)):
        OD.append(OD[i] + alphas[i] * widths[i])

    # After getting al the alphas and widths, we need to create a function that takes
    # as arguments the depth z and
