...
def mobility_low_field(
    ...
    Some trends:
        ...
        - At a given doping:
            - If l*t2 > t1, there's a maximum in the mobility at a certain temperature
...
class SotoodehMobilitySource(ParameterSourceBase):
    ...
    def get_parameter(self, material: str, parameter: str, **kwargs) -> Parameter:
        ...
        required = {"electron": ("T", "Nd"), "hole": ("T", "Na")}
        if not all((p in kwargs for p in required[carrier])):
            raise KeyError(
                f"Missing at least one required parameter for carrier '{carrier}': "
                f"{required[carrier]}"
            )
        ...
        N = kwargs["Nd"] if carrier == "electron" else kwargs["Na"]
