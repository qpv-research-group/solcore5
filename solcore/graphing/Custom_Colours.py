...
def colours(colour: str, type: str = "hex") -> Union[str, Tuple[float, ...]]:
    """colours(colour, type) :: Function identifies the name of the colour and returns
    the correct HEX or RGB key. Default return is in HEX.

    :param colour: string variable, name of colour as listed in the table of colours at;
    http://www.rapidtables.com/web/color/RGB_Color.htm :param type: string variable,
    choices of "hex or "rgb". If colour is not found an error is returned. :return: HEX
    string or RGB tuple
    """
    ...
    for Lines in raw_txt:
        ...
        if "$END$" in Lines:
            raise ValueError(
                "Colour name not found... Check http://www.rapidtables.com/web/color/RGB_Color.htm"
            )
...
def colour_cycle(name: str) -> Cycler:
    """colour_cycle(name, type) :: Function returns a cycler instance of a list of
    colours (in HEX) that can be used when plotting multi-plots.

    :param name: string variable matching the list of available colour cycles. Current
    list includes:                 'rainbow', 'origin_basic',
    'origin_system_colour_list'. :return: colour cycle instance
    """
    ...
    if name.lower() == "rainbow":  # 29 colours in shades of the rainbow...
        ...
        name.lower() == "origin_system_color_list"
    ):  # 13 colours defined by Origin's system colour list...
