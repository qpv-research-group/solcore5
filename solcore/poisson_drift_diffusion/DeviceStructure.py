...
def CreateDeviceStructure(
    ...
    comments="",
    repeat=1,
    substrate=DefaultMaterial,
    reflection=None,
):
    """Creates the structure of a device to be used in drift diffusion calculations.

    :param name: Name of the structure :param role: Role of the structure (eg. "device",
    "MQW", etc) :param T: Temperature :param layers: List containing all the layers of
    the structure. It can also be a Structure or a Junction. :param comments: A string
    with comments :param repeat: If the structure is actually something that is repeated
    many times, for example a quantum well :param substrate: A Solcore material defining
    the substrate of the structure. :param reflection: The reflection of the structure
    -> Not use from Solcore v4 :return: A dictionary-like object with all the
    ...
    output["absorption"] = (
        layers.absorbed if hasattr(layers, "absorbed") else lambda x: 0
    )
...
def AddLayers(device, layers):
    ...
    :param device: The device structure :param layers: A list with the layers to add
    :return: None
    """
    for layer in layers:
        NewLayer = {}
...
def RemoveLayer(device, i):
    ...
    :param device: the device structure :param i: the index of the layer to remove
    :return: None
    """
    del device["layers"][i]
    device["numlayers"] = device["numlayers"] - 1
...
def GetLayerProperties(layer, T):
    """Given a layer, get all the properties of that layer and the material it is made
    of at the given temperature.

    :param layer: The layer of interest :param T: The temperature :return: Dictionary
...
def LoadAbsorption(layer, T, wavelengths, use_Adachi=False):
    """Loads the absorption coefficient for the material of a given layer. It is only
    useful when solving QWs, since it is necessary to calculate an effective absorption
    coefficient before solving the opticalproperties of the structure using the BL, TMM
    or RCWA solvers.

    :param layer: The layer we want to get the absorption coefficient from. :param T:
    The temperature :param wavelengths: The wavelength we want the absorption at :param
    ...
    if use_Adachi:
        try:
            ...
            absorption = adachi_alpha.create_adachi_alpha(
                InLineComposition(layer), T=T, wl=wavelengths
            )[3]
        except:
            print(
                "Warning: Using experimental data to estimate the absorption coefficient of material: ",
                InLineComposition(layer),
            )
            absorption = ToSolcoreMaterial(
                layer["properties"]["composition"], T, execute=True
            ).alpha(wavelengths)
        ...
        try:
            ...
            if layer["properties"]["composition"]["material"] == "InGaAs":
                ...
                print(
                    "    >>>: We truncate the absorption at the bulk bandgap wavelength."
                )
                edge = 1240e-9 / (layer["properties"]["band_gap"] / q)
                edgeidx = np.abs(wavelengths - edge).argmin()
                absorption[edgeidx:] = 0
            ...
            try:
                ...
                print(
                    "Warning: No absorption information found for material {}. Setting it equal to zero.".format(
                        InLineComposition(layer)
                    )
                )
...
def SolveQWproperties(
    device,
    calculate_absorption=True,
    WLsteps=(300e-9, 1100e-9, 201),
    wavelengths=None,
    periodic=True,
    filter_strength=0.0,
    blur=None,
    ...
    modification of the bandeges due to strain, the efective mases and the absorption
    coefficient. Without calling this function, the structure is just a colection of
    layers with bulk-like properties.

    :param device: The device structure :param calculate_absorption: If absorption must
    be calculated :param WLsteps: wavelengths in which to calculate the absorption
    (input for np.linspace function) :param wavelengths: An array with the waveengths
    :param periodic: If it has to be assumed that the structure is perdiodic :param
    filter_strength: :param blur: :param blurmode: :param mode: :param use_Adachi:
    :param alpha_params: :return: A dictionary with the output of the Schrodinger
    solver.
    """
    ...
    QW = QWunit(
        ToStructure(device),
        substrate=ToSolcoreMaterial(device["substrate"], device["T"], execute=True),
    )
    output = QW.solve(
    ...
    for i in range(len(QW)):
        ...
        device["layers"][i]["properties"]["electron_affinity"] = QW[
            i
        ].eff_electron_affinity
        device["layers"][i]["properties"]["eff_mass_electron_Gamma"] = QW[
            i
        ].material.eff_mass_electron_Gamma
        device["layers"][i]["properties"]["eff_mass_hh_z"] = QW[
            i
        ].material.eff_mass_hh_z
        device["layers"][i]["properties"]["eff_mass_lh_z"] = QW[
            i
        ].material.eff_mass_lh_z
        ...
        device["layers"][i]["properties"]["ni"] = np.sqrt(
            QW[i].material.Nc
            * QW[i].material.Nv
            * np.exp(-QW[i].eff_band_gap / (kb * T))
        )

        if calculate_absorption:
            device["layers"][i]["properties"]["absorption"] = [
                QW.wl.tolist(),
                QW[i].material.absorption.tolist(),
            ]
