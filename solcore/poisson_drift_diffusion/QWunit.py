...
class QWunit(Structure):
    """Assembles a group of layers as a quantum well, calculating its properties as a
    whole.

    - It needs a minimum of 3 layers - There must be exactly one layer with the role =
    "well". - Top and bottom layers are assumed to be the barriers. - Anything not a
    barrier not a well will be considered as "interlayer".  Since the Poisson - Drift-
    Diffusion solver can not work with superlattices, there is no point of considering
    the solution of more than one QW. All QWs entering into the solver are independent,
    regardless of the width of the barriers.  To account for the possible presence of
    nearby QWs, perdiodic boundary conditions can be used to solve the Schrodinger
    equation. If barriers + interlayers are thick enought, this make no difference but
    if they are thin, it affects the number of confined levels. It does not 'create'
    minibands, though, so the results with thin barriers must be used with caution.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(self) < 3:
            print(
                "ERROR creating a QWunit: a minimum of 3 layers is necessary to make a unit. Only {0} found. ".format(
                    len(self)
                )
            )
            sys.exit()
        ...
        if "substrate" not in kwargs.keys():
            print(
                "ERROR creating a QWunit: There must be a substrate defined with the keyword 'substrate'. "
            )
            sys.exit()
    ...
    def RecalculateKP(self, index, mode):
        """Recalculates the band structure of the QW unit to account for modifications
        due to strain.

        This is already done in the Schrodinger solver, but it does not produce that
        information as output. We do it so here.
        """
        if mode == "kp8x8_bulk":
            kp_result = kp8x8_bulk(
                material=self[index].material,
                host_material=self.substrate,
                averaging_points=6,
            )
            ...
            Ec = (
                self[index].material.valence_band_offset + self[index].material.band_gap
            )
            Eb = c - max(hh, lh)
            ...
            # Idem
            strain_parameters = strain_calculation_parameters(
                self.substrate, self[index].material, should_print=False
            )
            Eb = (
                self[index].material.band_gap
                + strain_parameters.delta_Ec
                + max(strain_parameters.delta_Ehh, strain_parameters.delta_Elh)
            )
    ...
    def get_average_params(self, x, y):
        ...
        :param x: x values :param y: the parameter of interest at positions x :return:
        the average value of y in layer i
        """
    ...
    def RecalculateBandEdges(self, mode, SR):
        """The effective bandgap and effective affinity of each layer depends on the
        energy level, e.g.: the minimum energy for electrons is not the band edge of the
        conduction band, but the ground level. In this function we modify that creating
        an effective electron afinity and band gaps for all layers in the QW.

        For the barriers, the electron afinity and band gap is the same than in bulk,
        modified by the kp calculation, if necesary.
        """

        eff_c_shift = (
            np.where(
                SR["E"]["Ee"][0] > SR["potentials"]["Ve"],
                SR["E"]["Ee"][0],
                SR["potentials"]["Ve"],
            )
            - SR["potentials"]["Ve"]
        ...
        max_VB = np.where(
            SR["potentials"]["Vhh"] > SR["potentials"]["Vlh"],
            SR["potentials"]["Vhh"],
            SR["potentials"]["Vlh"],
        )
        ...
        for index in range(len(self)):
            ...
            self[index].material.__dict__["Ncc"] = (
                2 * (2 * pi * me * kb * self.T / h ** 2) ** 1.5
            )
            self[index].material.__dict__["Nvhh"] = (
                2 * (2 * pi * mhh * kb * self.T / h ** 2) ** 1.5
            )
            self[index].material.__dict__["Nvlh"] = (
                2 * (2 * pi * mlh * kb * self.T / h ** 2) ** 1.5
            )
    ...
    def RecalculateDensityOfStates(self, SR):
        """Calculates the effective density of states for each layer in the QW. The
        general rule is:

        - Barriers have the bulk density of states - QW have ALL the density of states
        asociated with the confined states + bulk density of states above the barrier -
        Interlayers have only the bulk density of states above the barrier  This
        simplification is similar to that in J. Nelson, M. Paxman, K. W. J. Barnham, J.
        S. Roberts, and C. Button, “Steady-state carrier escape from single quantum
        wells,” IEEE J. Quantum Electron., vol. 29, no. 6, pp. 1460–1468, 1993.  From a
        physical porint of view, it can certainly be done much better
        """
        ...
        for index in range(len(self)):
            ...
            Nqw = 2.0 / self[index].width * (Ncc / 2.0) ** (2.0 / 3.0) * Nqw
            ...
            Nqw = 2.0 / self[index].width * Nqw
            self[index].material.Nv = self[index].material.Nv + Nqw

    # ------
    def CalculateAbsorption(self, use_Adachi, SR):
        """If required, this function calculates the absorption of the QW, putting
        together the absorption of the confined levels and the absorption of the bulk.
        As with the density of states, the rules are:

        - Barriers have the bulk absorption - Interlayers have the bulk absorption from
        the barrier energy and zero below that - Wells have the absorption of the
        confined levels below the barrier energy and of the bulk above it. The
        calculation is similar to: C. I. Cabrera, J. C. Rimada, J. P. Connolly, and L.
        Hernandez, “Modelling of GaAsP/InGaAs/GaAs strain-balanced multiple-quantum well
        solar cells,” J. Appl. Phys., vol. 113, no. 2, p. 024512, Jan. 2013.
        """
        ...
        for index in range(len(self)):
            if use_Adachi:
                try:
                    self[index].material.absorption = adachi_alpha.create_adachi_alpha(
                        SolcoreMaterialToStr(self[index].material), T=self.T, wl=self.wl
                    )[
                        3
                    ]  # 0 = Energy, 1 = n, 2 = k, 3 = Absorption
                except:
                    self[index].material.absorption = self[index].material.alpha(
                        self.wl
                    )
            ...
            if self.labels[index] is "well":
                ...
                self[index].material.absorption = (
                    self[index].material.absorption
                    - self[index].material.absorption[edge_index]
                )
                self[index].material.absorption[edge_index:] = 0
                self[index].material.absorption[self[index].material.absorption < 0] = 0
                self[index].material.absorption = (
                    self[index].material.absorption
                    + SR["alpha"][1] * self[index].width / self.QW_width
                )
    ...
    def solve(
        ...
        self.__dict__["hhlevels"] = (
            max(SR["potentials"]["Vhh"]) - SR["E"]["Ehh"][:]
        ) / q
        self.__dict__["lhlevels"] = (
            max(SR["potentials"]["Vlh"]) - SR["E"]["Elh"][:]
        ) / q
    ...
    def GetEffectiveQW(
        ...
        """Considers the device as a QW and solves its properties, including the
        modification of the bandeges due to strain, the efective mases and the
        absorption coefficient. The output is a list of layers made with materials with
        the effective properties after considering all these effects in addition to the
        quantum confinement.

        :param device: The device structure :param calculate_absorption: If absorption
        must be calculated :param WLsteps: wavelengths in which to calculate the
        absorption (input for np.linspace function) :param wavelengths: An array with
        the waveengths :param periodic: If it has to be assumed that the structure is
        perdiodic :param filter_strength: :param blur: :param blurmode: :param mode:
        :param use_Adachi: :param alpha_params: :return: A dictionary with the output of
        ...
        for i in range(len(self)):
            ...
            inter = (
                ...
                * pi
                / (h ** 3 * c ** 2)
                * layer_mat.n(1240e-9 / (E / q)) ** 2
                * layer_mat.alphaE(E)
                * np.exp(-E / (kb * self.T))
                * E ** 2
            )
            Br = -np.trapz(
                np.nan_to_num(inter(1240e-9 / self.wl * q)), 1240e-9 / self.wl * q
            )
...
def SolcoreMaterialToStr(material_input):
    """Translate a solcore material composition into a string that is what the Adachi
    calculator needs."""
