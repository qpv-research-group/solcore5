...
def ProcessStructure(device, meshpoints, wavelengths=None):
    ...
    :param wavelengths: (Optional) Wavelengths at which to calculate the optical
    properties. :return: Dictionary containing the device structure properties as a
...
def equilibrium_pdd(junction, options):
    """Solves the PDD equations under equilibrium: in the dark with no external current
    and zero applied voltage.

    :param junction: A junction object :param options: Options to be passed to the
    solver :return: None
    """
    T = options.T
...
def short_circuit_pdd(junction, options):
    """Solves the devices electronic properties at short circuit. Internally, it calls
    Equilibrium.
