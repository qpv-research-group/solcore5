...
def iv_detailed_balance(junction, options):
    """Calculates the IV curve of a junction of kind "DB". This is a detailed balanced calculation
    of the IV curve of a PN junction characterized by a certain eqe, temperature and chemical
    potential (voltage). Normally, the eqe will be calculated based on a a given absorption edge
    and absorptance level resulting in a "top hat" type of eqe, but it can also be provided
    externally. By default, the solver uses the Boltzmann aproximation, although the full Planck
    ...
    try:
        ...
        raise AttributeError(
            "ERROR calculating the IV for the detailed balance Junction kind. "
            "Junction is missing one essential argument: {}".format(err)
        )

    if hasattr(junction, "back_reflector"):
        if junction.back_reflector:
            j01_pref = 1
    ...
    j01 = (
        j01_pref * q * n**2 * c
    )  # dark current prefactor: factor of 2 higher without a perfect
    # back reflector

    # We loop over the (internal) voltages to get the (internal) current.
    # The actual voltage applied is always smaller than Eg.
    junction.voltage = options.internal_voltages

    if mode == "boltzmann":
        # If Boltzmann, the calculation can be vectorized as the voltages is outside the integral.
        # No limit to V
        ...
        # If the full Planck equation is used, we need to loop over the voltages.
        # We limit them to the bandgap
...
def qe_detailed_balance(junction, wl):
    """Calculates the EQE and the IQE in the detailed balanced limit... which is equal to the
    absorptance and the absorbed fraction of the light since, by definition of detailed balanced,
    the carrier collection is perfect.
    ...
    junction.iqe = junction.absorptance  # shouldn't the IQE just be 1?
...
def absorptance_detailed_balance(junction):
    """Calculates the absorptance of the junction in the detailed balanced case. If it has not
    been calculated before, provided as input, it uses a "top hat" absorptance with the absorption
    edge at Eg and magnitude A.

    Note that while the junction.absorptance represents the potential fraction of light that can be
    absorbed at a given energy, the junction.absorbed represents the actual fraction of input light
    that is absorbed. For detailed balanced calculations, these two are equivalent to the IQE and
    EQE respectively because carrier collection is perfect, but that will not be the case, in
    general.
...
def surface_integral(junction, wl):
    ...
    # is junction.absorptance(wl) either = 1 or = 0?
    total_absorption = np.any(
        (junction.absorptance(wl) == 1, junction.absorptance(wl) == 0)
    )

    if np.all(junction.reflected(wl) == 0) and total_absorption:
        # in this case, R = 0, B = 0 above the bandgap,
        return 2 * np.pi * junction.absorptance(wl)
        ...
        neff = (1 + np.sqrt(Rn)) / (
            1 - np.sqrt(Rn)
        )  # inverse of Fresnel eqn at normal incidence
        ...
        for i in range(len(wl)):
            idx = np.where(cos_t > cos_tc[i])  # not totally internally reflected

            Rs = np.ones(steps)
            Rp = np.ones(steps)

            cos_i = np.sqrt(1 - neff[i] ** 2 * (1 - cos_t[idx] ** 2))  # refracted angle

            Rs[idx] = (
                (cos_i - neff[i] * cos_t[idx]) / (cos_i + neff[i] * cos_t[idx])
            ) ** 2  # Fresnel equations
            Rp[idx] = (
                (cos_t[idx] - neff[i] * cos_i) / (cos_t[idx] + neff[i] * cos_i)
            ) ** 2

            R[i, :] = (Rs + Rp) / 2

        # B = exp(-alpha*w) but it is not necessary to calculate alpha and w explicitly
        # intensity left after one pass
