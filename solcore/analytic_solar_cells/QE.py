...
from solcore import convert, eVnm, siUnits
from solcore.constants import electron_mass, hbar, kb, pi, q, vacuum_permittivity
...
def spectral_response_all_junctions(
    ...
    """Calculates the spectral response of any number of junctions using analytical
    diode equations as described in J. Nelson's book "The Physics of Solar Cells"
    (2003). All parameters must be in SI units. It only works for homojunctions.

    :param solar_cell: A structure object with one or more Junction objects and a few
    optional parameters. Each junction is made of a sequence of *Layers* (with a
    thickness, a material and a role) and the surface recombination velocities for
    electrons and holes. Optional attributes for the structure are:     - shading:
    (optional) Shading loses.     - reflectivity: (optional) Function that calculates
    the reflectivity as a     function of energy (in J). :param incident_light:
    (optional) 2D array containing the energies and the spectral power density (in
    photons m-2 J-1). :param energy: (optional) energies at which to calculate the QE.
    If not provided, the range of the incident ligth is used and if that is not defined,
    a "sensible" range is used. :param V: The voltage at which to perform the
    calculations (in V) :param verbose: If the information about the calculation must be
    printed. :return: A dictionary containing, as a function of energy:      -
    junctions: A list containing the QE data for each junction     - transmitted: The
    transmitted power density     - transmitted_fraction: The fraction of transmitted
    power     - passive_loss: The passive losses (light absorbed in the encapsulants, AR
    coatings, window layers, etc)     - reflected: reflected power density     -
    reflected_fraction: Fraction of reflected power     - e: The energy values
    """
    science_reference(
    ...
    # Get the energy range and incident spectrum. If they are not inputs, we create
    ...
    for layer_index, layer_object in enumerate(solar_cell):
        ...
        # For each junction, we calculate the spectral response
            ...
            # If there are window layers, passively absorbing light above the emitter,
            # we attenuate the intensity
            idx = 0
...
def calculate_junction_sr(junc, energies, bs, bs_initial, V, printParameters=False):
    """Calculates the total quantum efficiency, the QE splitted by regions, photocurrent
    and other parameters for a given junction at a given voltage.

    :param junc: The junction object :param energies: The energies at which to perform
    the calculation :param bs: The spectral power density reaching the junction :param
    bsInitial: The initial power density :param V: The voltage at which to perform the
    calculations (not implemented, yet) :param printParameters: If a list of all
    parameters must be printed :return:
    """
    ...
    if junc[idx + 1].role is "intrinsic":
        ...
        if junc[idx + 2].role is "base":
            ...
            raise RuntimeError(
                'ERROR processing junctions: A layer following the "intrinsic" layer '
                'must be defined as "base".'
            )
    ...
    # If there is no intrinsic region, we check directly the base
        ...
        raise RuntimeError(
            'ERROR processing junctions: A layer following the "emitter" must be '
            'defined as "intrinsic" or "base".'
        )
    ...
    # For the diffusion lenght, subscript n and p refer to the carriers, electrons
    # and holes
    if hasattr(junc, "ln"):
        ln = junc.ln
    else:
    ...
    # For the diffusion coefficient, n and p refer to the regions, n side and p side.
    # Yeah, it's confusing...
    if hasattr(junc, "mup"):
        dp = junc.mup * kb * T / q
    else:
    ...
    # And now we account for the possible applied voltage, which can be, at most,
    # equal to Vbi
    V = min(Vbi, V)
    Vbi = Vbi - V
    ...
    if not hasattr(junc, "wp") or not hasattr(junc, "wn"):
        ...
        if (
            ...
            science_reference(
                ...
                "Sze: The Physics of Semiconductor Devices, 2nd edition, John Wiley & "
                "Sons, Inc (2007)",
            )
    ...
    # we have an array of alpha values that needs to be interpolated to the right
    # energies
    ...
    # hereby we define the subscripts to refer to the layer in which the current is
    # generated:
    if pn_or_np == "pn":
    ...
    # These might not be the right lifetimes. Actually, they are not as they include
    # all recombination processes, not
    # just SRH recombination, which is what the equation in Jenny, p159 refers to.
    # Let´ leave them, for now.
    lifetime_n = ln ** 2 / dn
...
|   minority carrier (electron) effective mass: {relativeEffectiveMassE:.3f} (relative) 
{mEff_e:.3e} kg (absolute)
|   minority carrier (electron) diffusivity = {dp:.3e} m2/s
|   depletion width: {wpNm:.3f} nm
|   charge in depletion region: {pDepletionCharge:.3e} C m-2 ({pQ:.3e} C cm-2)
{iRegionString}
| n region: {xnUm:.2f} um {nMatrialString}
|   Nd = {Nd:.3e} m-3 ({NdCM:.3e} cm-3)
|   minority carrier (hole) diffusion length: {lnum:.3f} um
|   minority carrier (hole) effective mass: {relativeEffectiveMassH:.3f} (relative) 
{mEff_h:.3e} kg (absolute)
