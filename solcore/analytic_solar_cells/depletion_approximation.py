...
def iv_depletion(junction, options):
    ...
    if options.light_iv:
        ...
        g = junction.absorbed
        wl_sp, ph_sp = options.light_source.spectrum(output_units='photon_flux_per_nm')
        wl = options.wavelength * 1e9
        ph = np.interp(wl, wl_sp, ph_sp)
    ...
    # And, finally, we ouput the currents
...
def qe_depletion(solar_cell, args):
    pass

# def print_info(parameterDictionary):
#     print("""\n
...
#     if options.print_parameters_DA:
...
# |   field: {fieldMax:.3e} V/m""".format(**{"iMatrialString": str(iRegion.material),
