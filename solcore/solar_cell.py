...
class SolarCell(Structure):
    """This class is almost identical to the basic Structure class in Solcore (it is a
    subclass of it, actually) but implementing some default parameter values and control
    about the types of layers.

    It should work anywhere where a Structure object works.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        :param layers: A list with the layers to add. The layers might include
        individual Layer objects of whole Junction objects. :param T: Temperature.
        :param cell_area: The area of the cell. :param reflectivity: Function that
        calculates the reflectivity as a function of energy. :param shading: Shading
        losses due to the front metal contacts. :param substrate: Substrate of the solar
        cell. :param R_series: Series resistance of the structure :param kwargs: Other
        possible attributes.
        """

        assert isinstance(
            layers, list
        ), "Layers must be provided inside a list, even if it is just one layer."
    ...
    def sort_layer_type(self, layer, i):
        """Sorts the layer in different categories, depending on its type, and keeps
        record on the indices of that type of layer.

        :param layer: The layer to check :param i: The index of that layer :return: None
        """
    ...
    def append(self, new_layer, layer_label=None, repeats=1):
        ...
        :param new_layer: The layer to append. :param layer_label: An optional label for
        that layer. :param repeats: Number of times to repeat add the layer. :return:
        None
        """
    ...
    def append_multiple(self, layers, layer_labels=None, repeats=1):
        ...
        :param layers: A list with the layers to append. :param layer_labels: An
        optional list with the labels. If present, it must have the same length that the
        layers list. :param repeats: Number of times to add this set of layers. :return:
        None
        """
        ...
        if layer_labels is not None:
            assert len(layers) == len(layer_labels), (
                "When using 'layer_labels' keyword a label must be specified for "
    ...
    def update_junction(self, junction, **kwargs):
        ...
        :param junction: The junction to update. :param kwargs: The attributes to
        update. :return: None
        """
        ...
        try:
            ...
            print(
                "ERROR updating junction: The junction index must be {} or less.".format(
                    len(self.junction_indices)
                )
            )
