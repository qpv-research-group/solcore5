...
class interp1d(object):
    ...
    def __init__(
        ...
        Description ----------- x and y are arrays of values used to approximate some
        function f:     y = f(x) This class returns a function whose call method uses
        linear interpolation to find the value of new points.  Parameters ---------- x :
        array     A 1D array of monotonically increasing real values.  x cannot include
        duplicate values (otherwise f is overspecified) y : array     An N-D array of
        real values.  y's length along the interpolation     axis must be equal to the
        length of x. kind : str or int     Specifies the kind of interpolation as a
        string ('linear',     'nearest', 'zero', 'slinear', 'quadratic, 'cubic') or as
        an integer     specifying the order of the spline interpolator to use. axis :
        int     Specifies the axis of y along which to interpolate. Interpolation
        defaults to the last axis of y. copy : bool     If True, the class makes
        internal copies of x and y.     If False, references to x and y are used. The
        default is to copy. bounds_error : bool     If True, an error is thrown any time
        interpolation is attempted on     a value outside of the range of x (where
        extrapolation is     necessary).     If False, out of bounds values are assigned
        fill_value.     By default, an error is raised. fill_value : float     If
        provided, then this value will be used to fill in for requested     points
        ...
        if kind in ["zero", "slinear", "quadratic", "cubic"]:
            order = {"nearest": 0, "zero": 0, "slinear": 1, "quadratic": 2, "cubic": 3}[
                kind
            ]
            kind = "spline"
    ...
    def __call__(self, x_new: np.ndarray) -> np.ndarray:
        ...
        Parameters ---------- x_new : number or array     New independent variable(s).
        Returns ------- y_new : ndarray     Interpolated value(s) corresponding to
        x_new.
        """
    ...
    def _check_bounds(self, x_new: np.ndarray) -> np.ndarray:
        ...
        Parameters ---------- x_new : array  Returns ------- out_of_bounds : bool array
...
class BilinearInterpolation(object):
    ...
    def __init__(
        ...
        assert x_s[0] == z_s[0], (
            "The number of rows in z (which is %s), must be the same as number of elements in x (which is %s)"
            % (x_s[0], z_s[0])
        )
        assert y_s[0] == z_s[1], (
            "The number of columns in z (which is %s), must be the same as number of elements in y (which is %s)"
            % (y_s[0], z_s[1])
        )
    ...
    def __call__(self, xvalue: float, yvalue: float) -> np.ndarray:
        ...
        try:
            ...
            if (
                type(xvalue) == list
                or type(xvalue) == tuple
                or type(xvalue) == type(np.array([]))
            ):
                constructed_list = []
...
if __name__ == "__main__":
    ...
    x = np.array([400.0, 600.0, 601, 1000.0])

    # Some angle values
    y = np.array([0.0, np.pi / 4, np.pi / 2])
