...
def solve_pv_module(
    ...
    """Calculate the IV curve of a PV module made of a certain number solar_cells
    connected in series in a single string. A certain dispersion to the distribution of
    photocurrents among cells and shadowing losses can be added to ahoeve more realistic
    results.

    :param solar_cell: A solar cell object containing all the junctions :param options:
    A state object with all the options needed to solve the solar cell IV :param
    totalcells: Total number of cells in a string :param bias_start: Initial voltage of
    the IV curve of the module :param bias_end: Final voltage of the IV curve of the
    module :param bias_step: Step for the bias :param jscSigma: Width of the
    distribution of short circuit currents around the ideal (eg: 0.02 = 2%). Default is
    set to 0.02% to aid convergence :param shading: Array containing the shading loses
    for each cell. It can be a 2D array (0 = no shading, 1 = full shading) :return: A
    tuple with the array of voltages, currents, a list with the Isc of each junction in
    each cell and the raw output from SPICE.
    """
    ...
    for i in range(totaljuncs):
        ...
        try:
            ...
            rsh_array[i] = (
                min(solar_cell(i).R_shunt / area, 1e14)
                if hasattr(solar_cell(i), "R_shunt")
                else 1e14
            )
        except AttributeError as err:
            raise AttributeError(
                "ERROR in PV module: Junction is missing one essential argument. {}".format(
                    err
                )
            )
    ...
    jsc_random_normal = abs(
        1 - max(jsc_random_normal) * jscSigma + jscSigma * jsc_random_normal
    ) * (1 - shade)
    ...
    for n in range(1, totalcells + 1):
        ...
        spiceout = "r{0} {1} {2} {3}\n".format(
            2 * junctionCounter - 1, nodeCounter + 1, nodeCounter, Rs
        )
        nodeCounter = nodeCounter + 1
        spice_file = spice_file + spiceout
        ...
        spiceout = "d{0} {1} {2} bypassdiode\n".format(
            2 * junctionCounter + 1, lowerBypassConnection, nodeCounter
        )
        spice_file = spice_file + spiceout
...
def spice_junction(jc, nc, isc, j01, j02, n1, n2, Eg, rsh):
    """Creates the string representation in SPICE of the junciton defined by the input
    values.

    :param jc: junction counter :param nc: node counter :param isc: Short circuit
    current :param j01: Reverse saturation current corresponding to ideality factor n1
    :param j02: Reverse saturation current corresponding to ideality factor n2 :param
    n1: Ideality factor, typically = 1 :param n2: Ideality factor, typically = 2 :param
    Eg: Bandgap of the material the junction is made off :param rsh: Shunt resistance in
    the junction :return: A string representation of the junciton in SPICE
    """
