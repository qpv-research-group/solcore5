...
def solve_quasi_3D(
    ...
    :param solar_cell: A solar cell object :param injection: 2D array indicating the
    (optical) injection mask :param contacts: 2D array indicating the electrical
    contacts :param options: Options for the 1D solar cell solver :param Lx: Pixel size
    in the X direction :param Ly: Pixel size in the Y direction :param h: Height of the
    metal fingers :param R_back: Resistance back contact :param R_contact: Contact
    resistance :param R_line: Resistivity metal fingers :param bias_start: Initial
    voltage (V) :param bias_end: Final voltage (V) :param bias_step: Voltage step (V)
    :return: A tuple with:      - V [steps + 1] : 1D Array with the external voltages -
    I [steps + 1] : 1D Array with the current at all external V     - Vall [xnodes,
    ynodes, 2 * junctions, steps + 1] : 4D Array with the voltages in all nodes, at all
    external V     - Vmet [xnodes, ynodes, steps + 1] : 3D Array with the voltages in
    the metal nodes, at all external V
    """
    ...
    for i in range(totaljuncs):
        ...
        rsh_array[i] = (
            min(solar_cell(i).R_shunt, 1e16)
            if hasattr(solar_cell(i), "R_shunt")
            else 1e16
        )
        rshTop_array[i] = (
            max(solar_cell(i).R_sheet_top, 1e-16)
            if hasattr(solar_cell(i), "R_sheet_top")
            else 1e-16
        )
        rshBot_array[i] = (
            max(solar_cell(i).R_sheet_bot, 1e-16)
            if hasattr(solar_cell(i), "R_sheet_bot")
            else 1e-16
        )
        ...
        try:
            ...
            raise AttributeError(
                "ERROR in quasi-3D solver: Junction is missing one essential argument. {}".format(
                    err
                )
            )
    ...
    for i in solar_cell.tunnel_indices:
        rseries_array[j] = (
            max(solar_cell[i].R_series, 1e-16)
            if hasattr(solar_cell[i], "R_series")
            else 1e-16
        )
        j += 1
...
def create_node(
    ...
    """Creates a node of the solar cell, meaning all the circuit elements at an XY
    location in the plane. This includes all the diodes, resistances and current sources
    for all the junctions at that location.

    :param type: The type of the node, 'Normal', 'Finger' or 'Bus' :param idx: Index
    with the location in the X direction :param idy: Index with the location in the Y
    direction :param Lx: Pixel size in the X direction :param Ly: Pixel size in the Y
    direction :param Isc: Array of Isc for each of the junctions :param topLCL: Array of
    resistances of the top lateral conductive layer :param botLCL: Array of resistances
    of the bottom lateral conductive layers :param rshunt: Array of Rshunt for each of
    the junctions :param rseries: Array of Rseries for each of the junctions :param
    xMetalTop: Resistance of the metal in the X direction :param yMetalTop: Resistance
    of the metal in the Y direction :param contact: Contact resistance :return: The node
    define in SPICE file as a string.
    """
    node = ""
    for j in range(len(Isc)):
        ...
        if j + 1 == len(Isc):
            ...
            locLow = (
                "t_" + str(j + 1) + "_" + str(idx).zfill(3) + "_" + str(idy).zfill(3)
            )
        ...
        if j == 0 and type == "Finger":
            ...
            rmetalX = "RbusX{0}to{1} m_{0} m_{1} {2}\n".format(
                loc, locXR, xMetalTop / s
            )
            rmetalY = "RbusY{0}to{1} m_{0} m_{1} {2}\n".format(
                loc, locYR, yMetalTop * s
            )
            rext = ""
...
def create_header(I01, I02, n1, n2, Eg, T=20):
    """Creates the header of the SPICE file, where the diode models, the temperature and
    the independent voltage source are defined.

    :param I01: Array of I01 for each of the junctions :param I02: Array of I02 for each
    of the junctions :param n1: Array of n1 for each of the junctions :param n2: Array
    of n2 for each of the junctions :param Eg: Array of Eg for each of the junctions
    :param T: Temperature of the device :return: The header of the SPICE file as a
    string.
    """
...
def solve_circuit_quasi3D(
    ...
    :param vini: Initial voltage (V) :param vfin: Final voltage (V) :param step: Voltage
    step (V) :param Isc: Array of Isc for each of the junctions :param I01: Array of I01
    for each of the junctions :param I02: Array of I02 for each of the junctions :param
    n1: Array of n1 for each of the junctions :param n2: Array of n2 for each of the
    junctions :param Eg: Array of Eg for each of the junctions :param Rshunt: Array of
    Rshunt for each of the junctions :param Rseries: Array of Rseries for each of the
    junctions :param injection: 2D array indicating the (optical) injection mask :param
    contacts: 2D array indicating the electrical contacts :param RsTop: Array of sheet
    resistance on the top for each of the junctions :param RsBot: Array of sheet
    resistance on the bottom for each of the junctions :param Rline: Resistance of the
    metal fingers :param Rcontact: Contact resistance :param Lx: Pixel size in the X
    direction :param Ly: Pixel size in the Y direction :return: A tuple with:      - V
    [steps + 1] : 1D Array with the external voltages     - I [steps + 1] : 1D Array
    with the current at all external V     - Vall [xnodes, ynodes, 2 * junctions, steps
    + 1] : 4D Array with the voltages in all nodes, at all external V     - Vmet
    [xnodes, ynodes, steps + 1] : 3D Array with the voltages in the metal nodes, at all
    external V
    """
    ...
    for xx in x:
        for yy in y:
            ...
            if not shadow[xx, yy]:
                ...
                SPICEbody = SPICEbody + create_node(
                    "Normal",
                    xx,
                    yy,
                    Lx,
