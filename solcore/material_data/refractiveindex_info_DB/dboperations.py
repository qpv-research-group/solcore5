...
_riiurl = "https://refractiveindex.info/download/database/rii-database-2025-02-23.zip"
...
class Database:
    ...
    def create_database_from_folder(self, yml_database_path, interpolation_points=100):
        create_sqlite_database(yml_database_path, self.db_path,
    ...
    def get_material(self, pageid):
        ...
        if pagedata is None:
            ...
            return DBMaterial.FromLists(pagedata, wavelengths_r=wavelengths_r,
    ...
    def get_material_csv(self, pageid, output="", folder=""):
        ...
        # print(matInfo)
        if output == "":
            output = ",".join([str(matInfo['pageid']), matInfo['shelf'], matInfo['book'],
                               matInfo['page']]) + ".csv"
        if folder != "":
    ...
    def get_material_txt(self, pageid, output="", folder=""):
        ...
        # print(matInfo)
        if output == "":
            output = "_".join([str(matInfo['pageid']), matInfo['shelf'], matInfo['book'],
                               matInfo['page']]) + ".txt"
        if folder != "":
            output = folder + os.sep + output
...
def extract_entry_list(db_path):
    ...
    try:
        ...
        with open(os.path.join(referencePath, os.path.normpath("catalog-nk.yml")), "r") as f:
            catalog = yaml.safe_load(f)
                ...
                if 'DIVIDER' not in b:
                    ...
                    for p in b['content']:
                        if 'DIVIDER' not in p:
                            page = Page(p['PAGE'],
                                        p['name'],
                                        os.path.join(os.path.join(referencePath, 'data'),
...
def create_sqlite_database(refractiveindex_db_path, new_sqlite_db, interpolation_points=100):
    ...
    conn.commit()
    conn.close()
    _populate_sqlite_database(refractiveindex_db_path, new_sqlite_db,
                              interpolation_points=interpolation_points)
