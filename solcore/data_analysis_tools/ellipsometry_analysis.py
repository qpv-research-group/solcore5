...
from solcore.absorption_calculator import OptiStack, calculate_ellipsometry


class EllipsometryData:
    """This class is used to open and arrange in a sensible way ellipsometry datafiles
    created with the WVASE software."""

    def __init__(self, path: str) -> None:
        """Creates an ellipsometry data object and loads data into it the ellipsometry
        data from a file.
    ...
    def _load_data(self, path: str) -> None:
        ...
        :param path: Path to the file :return: None
        """
    ...
    def append_data(self, path: str) -> None:
        """Ellipsometry data from more than one source can be combined in a single
        object. This function does that.
    ...
    def plot(self, log: bool = True):
        ...
        :param log: True/False for ploting logarithmic X scale. :return: None
        """
...
class EllipsometryFitter:
    """Class that contains all the tools to fit the properties of a OptiStack model to
    ellipsometry data."""
    ...
    def set_variables(self, vars: Collection) -> None:
        """Sets the variables that will be used during the fitting. The input variable
        "vars" is a list with as many elements as layers in the model. Each of these
        elemenets is, in turn, a list with the names of the free.

        variables of that layer. If the layers are defined by a dielectric model -
        rather than experimental n and k data the variable names other than the
        thickness and e_inf must be inside a list.  vars =  [   [], ['width', ['A1',
        'A2']],             [['A1', 'E1'], ['A1', 'Br1']]         ] The above variable
        list indicate that there are no free variables in the first layer, that the
        width and two amplitudes of the first model are free variables in the second
        layer, and for the first layer, that the free variables are the amplitude and
        energy of the first model, and the amplitude and broadening of the second one.
        The models are read in order. If there are free variables in the second model
        but not in the first one, then an empty list must be used. The last layer in the
        above example will look, in this case:  [[], ['A1', 'Br1']  :param vars: The
        list of free variables. :return: None.
        """
    ...
    def update_variables(self, v: List[float]) -> None:
        """Updates the stack with the values of the variables contained in v. The
        meaning of each of the elements in v is given by the definitions in self.vars.

        :param v: An array with the new values of the variables. :return: None
        """
        k = 0
    ...
    def get_variables(self) -> List[float]:
        ...
        :return: The values of the free variables, in the order they appear in
        self.vars.
        """
        out = []
    ...
    def set_range(self, wavelength_range: List[float]) -> None:
        """Set the wavelengths in which to make the ellipsometry calculations. Ideally,
        it should be dense enought to capture all the features of the dielectric
        functions but sparse enought so the calculations are fast.
    ...
    def mse(self, v: List[float] = None) -> np.ndarray:
        """Calculates the mean-squared error between the experimental data and the
        modelled one. If the vars input is given, the variables are first updated and
        then the MSE is calculated.

        Since the experimental data might have a lot of wavelength points, we use a
        reduced number of them as defined in self.range in order to speed up the
        calculation of the ellipsometric parameters. The modelled data is then
        interpolated to the experimental one in order to calculate the MSE. While this
        will lead to higher uncertainties when the experimental data varies rapidly, it
        makes the calculation much faster.  :param v: The free variables to update.
        :return: The mean-squared error between the experimental data and the modelled
        one
        """
        assert (
    ...
    def fit(self, show: bool = True) -> None:
        """Calculate the best fit of the model to the data by minimising the mean-
        squared error using as free varaibles those defined in self.vars. The hessian
        matrix is used to calculate the 90% confident limits and the correlation matrix.

        :param show: If the result of the fit must be printed. :return:
        """
    ...
    def calculate_uncertainties(self) -> None:
        """Calculates the uncertainties in the calculated optimum parameters. For that,
        it uses the inverse of the hessian (or curvature matrix) C provided by the
        fitting algorithm. Two quantities are defined:

        Error for parameter i (= 90% confidence limit): .. math:: Err_{ii} =
        1.65\\sqrt{MSE*C_{ii} }  Correlation matrix: .. math:: CM_{ij} = \\frac{C_{ij}}
        {\\sqrt{C_{ii}} \\sqrt{C_{jj}}}  The diagonals of the CM are always equal to one
        and the off-diagonal terms represent how correlated are the corresponding
        variables. Highly correlated variables (absolute values close to 1) mean that
        the minimum of the MSE is not very sharp with respect to changes between those
        two variables.  The above information is stored in self.errors and
        self.correlations, respectively.  :return: None
        """
    ...
    def plot(self, log: bool = True) -> None:
        ...
        :param log: If the X scale should be log. :return: None
        """
