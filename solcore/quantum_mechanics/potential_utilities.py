...
import numpy as np
from numpy import array, ones, sqrt, trapz
...
def tridiag_euler(V, z, m, periodic=False, num_eigenvalues=10, quasiconfined=0):
    ...
    axis = (
        hbar ** 2 / (4 * dz ** 2) * (1 / m_a + 2 / m_b + 1 / m_c) + V
    )  # d_j from Frensley, W. R. (1991)
...
def schroedinger_solve(
    ...
    Arguments:      x -- spatial grid     V -- potential     m -- effective mass
    Keywords:      electron -- whether the wavefunctions describe electrons or holes
    (default: True)     num_eigenvalues -- Number of eigenvalues to calculate (default =
    10)     periodic   -- not to sure what this does (default: False)
    """
...
def __potentials_to_wavefunctions_energies_internal(
    x,
    Ve,
    me,
    Vhh,
    ...
    Arguments:      x -- spatial grid     Ve -- electron potential     me -- electron
    effective mass     Vlh -- light hole potential     mlh -- light hole effective mass
    Vhh -- heavy hole potential     mhh -- heavy hole effective mass   Keywords:
    num_eigenvalues -- Number of eigenvalues to calculate (default = 10)     periodic --
    not to sure what this does (default: False)
    """

    Ee, psi_e = schroedinger_solve(
        x,
        Ve,
        me,
        num_eigenvalues,
    ...
    if filter_strength != 0:
        ...
        ), "Need to provide structure to find well regions for filtering"
        print("filtering")
...
def discard_unconfined_energy(E, psi, V, quasiconfined):
    ...
    try:
        ...
        return ([], [])
    print(
        "Wavefunction filter removed %d state%s."
        % (before - len(E), "" if (before - len(E)) == 1 else "s")
    )
    return E, psi
    ...
    try:
        ...
        E, psi = zip(
            *[
                (E_i, psi_i)
                for (E_i, psi_i) in zip(E, psi)
                if np.trapz(psi_i[indx] ** 2, x=x[indx]) / np.trapz(psi_i ** 2, x=x)
                >= threshold
            ]
        )
    except ValueError as exception:
...
def potentials_to_wavefunctions_energies(
    x,
    Ve,
    me,
    Vhh,
