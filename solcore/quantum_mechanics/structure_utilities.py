...
from solcore.quantum_mechanics.kp_QW import kp4x4, kp6x6
...
def assemble_qw_structure(
    repeats,
    well,
    bulk_l_top,
    bulk_l_bottom,
    barrier,
    well_interlayer=None,
    structure_label="QW Structure",
    shift_wells=0,
):
    ...
    if well_interlayer:
        qw_structure.append_multiple(
            [well_interlayer, well, well_interlayer, barrier],
            layer_labels=["interlayer", "well", "interlayer", "barrier"],
            repeats=repeats - 1,
        )
        qw_structure.append_multiple(
            [well_interlayer, well, well_interlayer, half_barrier, bulk_l_bottom],
            layer_labels=["interlayer", "well", "interlayer", "half barrier", "bulk"],
        )
    else:
        qw_structure.append_multiple(
            [well, barrier], layer_labels=["well", "barrier"], repeats=repeats - 1
        )
        qw_structure.append_multiple(
            [well, half_barrier, bulk_l_bottom],
            layer_labels=["well", "half barrier", "bulk"],
        )
...
def locate_regions(x, structure, region_sought="well"):
    ...
    well_structure_indices = set(
        [
            item
            for item in range(len(structure.labels))
            if structure.labels[item] == region_sought
        ]
    )  # <---- Uses new labels in the structure object
    well_z_indices = np.array([], dtype=np.dtype(int))
    for i, layer in enumerate(structure):
        if well_structure_indices.issuperset([i]):
            well_z_indices = np.hstack(
                (
                    well_z_indices,
                    np.where((depth <= x) * (x <= depth + layer.width) == True)[0],
                )
            )
        depth += layer.width
...
def text_render(structure, resolution=100):
    ...
    barrier = set(locate_regions(x, structure, "barrier")) | set(
        locate_regions(x, structure, "half barrier")
    )
...
def structure_to_potentials(
    ...
    """Discretizes the structure as a function of the position, providing the potential
    for the electrons, HH, LH and SO bands as well as the Luttinger parameters and the
    effective masses.

    The result depend on the chosen mode:     - kp4x4, calculates the bands and
    effective masses at k=0 coupling the HH and     LH bands     - kp6x6, calculates the
    bands and effective masses at k=0 coupling the HH, LH     and SO bands     -
    kp8x8_bulk, calculates the four bands and fit the effective masses with a parabola
    around k=0     - strain, just shifts the bands according to the strain - relaxed, do
    nothing and things are calculated as if we had the bulk, unstrained materials Notice
    that although the kp8x8_bulk couples the 4 (8) bands, the effective mass is the
    result of a fitting around k=0. Therefore, it is not valid to solve 1D problems, but
    just a bulk-like problem under a parabolic aproximation Additionlly, the band
    structure can be blured - to simulate the intermixing of materials - as well as
    being under an electric field.
    """

    # We calculate the mesh based on the minimum feature of the structure and the
    # number of points desired for that
    # feature

    available_modes = ["kp6x6", "kp4x4", "kp8x8_bulk", "strain", "relaxed"]
    assert mode in available_modes, "ERROR: Calculation mode must be {}".format(
        available_modes
    )
    ...
    # At each position of the structure, we have to calculate the effective mass of
    # electrons and holes, and the band
    # edges.
    for i, layer in enumerate(structure):
        ...
        mhh_p[positions] = 1.0 / (g1[positions] - 2 * g2[positions]) * m0
        mhh_t[positions] = 1.0 / (g1[positions] + 2 * g2[positions]) * m0
        mlh_p[positions] = 1.0 / (g1[positions] + g2[positions]) * m0
        mlh_t[positions] = 1.0 / (g1[positions] - g2[positions]) * m0
        ...
        try:
            ...
            print(
                "There was a problem converting the structure to a potential because "
                "some information was missing from the structure state."
            )
            print(
                "Try calling the function 'align_heterostructure_using_Vurgaftman' "
                "before calling 'structure_to_potentials'."
            )
            sys.exit()
        ...
        # Now we modify the band profile and effective masses according to the chosen
        # mode:
        # - kp4x4, calculates the bands and effective masses at k=0 coupling the HH and
        # LH bands
        # - kp6x6, calculates the bands and effective masses at k=0 coupling the HH, LH
        # and SO bands
        # - kp8x8_bulk, calculates the four bands and fit the effective masses with a
        # parabola around k=0
        # - strain, just shifts the bands according to the strain
        # - relaxed, do nothing and things are calculated as if we had the bulk,
        # unstrained materials
        if mode == "kp6x6":
            # The band edges are shifted and the effective mass of the LHs also changes.
            c, hh, lh, so, mc_kp, mhh_p_kp, mlh_p_kp, mso_p_kp, mhh_t_kp, mlh_t_kp, mso_t_kp = kp6x6(
            ...
            # The band edges are shifted but the effective masses around k=0 are not
            # affected.
            c, hh, lh, mc_kp, mhh_p_kp, mlh_p_kp, mhh_t_kp, mlh_t_kp = kp4x4(
                layer.material, substrate
            )
            ...
            # The band edges and the effective mass of Electrons, HH, LH and the SO
            # holes changes.
            ...
            strain_parameters = strain_calculation_parameters(
                substrate, layer.material, SO=True
            )
