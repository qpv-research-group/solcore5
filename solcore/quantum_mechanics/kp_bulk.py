...
def eight_band_strain_hamiltonian(
    kx,
    ky,
    kz,
    Ev0,
    ...
    """Hamiltonian to calculate cb, hh, lh, so bands and include strain for the biaxial
    (along [001]) special case.
    ...
    Ehh = lambda kx, ky, kz, exx, eyy, ezz: Ev0 - (
        P(kx, ky, kz, exx, eyy, ezz) + Q(kx, ky, kz, exx, eyy, ezz)
    )
    Elh = lambda kx, ky, kz, exx, eyy, ezz: Ev0 - (
        P(kx, ky, kz, exx, eyy, ezz) - Q(kx, ky, kz, exx, eyy, ezz)
    )
...
def kp_bands(
    ...
    return_so=False,
    graph=False,
    fit_effective_mass=False,
    effective_mass_direction="X",
    additional_traverse=None,
):
    """:param material:

    :param host_material: :param kx: :param ky: :param kz: :param return_so: :param
    graph: :param fit_effective_mass: :param effective_mass_direction: :param
    additional_traverse: :return:
    """
    ...
    # Strain parameters
    exx = (
        -(material.lattice_constant - host_material.lattice_constant)
        / material.lattice_constant
    )
    ...
    result = eight_band_strain_hamiltonian(
        kx, ky, kz, Ev0, Ec0, exx, ezz, me_eff, g1, g2, g3, a0, Delta, ac, av, b, Ep
    )
    so, lh, hh, c = result[::2]
    ...
    if fit_effective_mass:
        ...
        bands_at_gamma = eight_band_strain_hamiltonian(
            k_cGx,
            k_cGy,
            k_cGz,
            Ev0,
        ...
        bands_at_fitpoint = eight_band_strain_hamiltonian(
            k_fx,
            k_fy,
            k_fz,
            Ev0,
    ...
    if graph:
        ...
        allk, x_graph, xticks = traverse_brillouin(
            a0, traverse_order=traverse_order, steps=300
        )  # ()
        ...
        for i, b in enumerate(zip(*bands)):
            ...
            if fit_effective_mass:
                g.append(
                    GraphData(
                        x_graph,
                        effective_mass_energy(
                            x_graph / fitpoint_x * k_max, effective_masses[i], e0s[i]
                        )
                        / electron_charge,
                        color="red",
                        linewidth=2,
...
def KPbands(
    ...
    """New version of the above function that produces either the band edges or the full
    bands in one specifc direction from Gamma.

    - Calculation of the full Brillouin zone is not allowed. After all, KP is only valid
    near k=0 - The calculation of the effective masses is done externally by another
    function - The direction to calculate the bands is provided either with directory
    angles or a directory vector - The number of points of the bands and the fraction up
    to the border of the Brilluin zone in the given direction are also inputs - The
    default direction is X, as given by the angles theta (t) and phi (p)
    """
    ...
    if return_edges_only:
        ...
        result = eight_band_strain_hamiltonian(
            0, 0, 0, Ev0, Ec0, exx, ezz, me_eff, g1, g2, g3, a0, Delta, ac, av, b, Ep
        )
        so, lh, hh, c = result[::2]
...
def fit_effective_masses(bands, material, host_material, plot_result=False, dk=0.026):
    ...
    for i in range(
        1, len(bands), 2
    ):  # Bands are duplicated (at least for now) so we only fit half of them
        band = bands[i]
...
def average_inplane_effective_mass(
    material, host_material, averaging_points=10, dk=0.026
):
    """Calculates the average in-plane effective mass for the four bands, assuming
    always the XY plane."""
...
def kp8x8_bulk(material, host_material, averaging_points=10, dk=0.026):
    """Combines the above functions to provide a compact result of the band edges and
    the effective masses."""

    mc, mhh, mlh, mso = average_inplane_effective_mass(
        material, host_material, averaging_points=averaging_points, dk=dk
    )
