...
def L(x, centre, hwhm):
    return (
        1 / pi * (0.5 * hwhm) / ((x - centre) ** 2 + (0.5 * hwhm) ** 2)
...
def prepare_wavefunction_data(
    ...
    for e, psi in zip(E, Psi):
        ...
        data.append(
            GraphData(
                trimmed_x, e + norm_psi * scale * invert, color=color, **linekwargs
            )
        )
    return data
...
def wavefunctions_graph(
    ...
    **kwargs
    ...
    if Ee is not None:
        data.extend(
            prepare_wavefunction_data(
                x, Ee, psi_e, trim_levels_beyond, linewidth, "blue", scale, label="e"
            )
        )
    if Ehh is not None:
        data.extend(
    ...
    if Elh is not None:
        data.extend(
            prepare_wavefunction_data(
                x, Elh, psi_lh, trim_levels_beyond, linewidth, "red", scale, label="lh"
            )
        )
...
def potentials_graph(x, Ve=None, Vhh=None, Vlh=None, color="grey", **kwargs):
    ...
    if Ve is not None:
        data.append(GraphData(x, Ve, color=color, linewidth=2, label="Ve"))
    if Vlh is not None:
        data.append(
            GraphData(x, Vlh, color=color, linewidth=2, dashes=[1, 1], label="Vlh")
        )
    if Vhh is not None:
        data.append(GraphData(x, Vhh, color=color, linewidth=2, label="Vhh"))
...
def schrodinger_graph(schrodinger_result, **kwargs):
    ...
    w = wavefunctions_graph(
        figure=potential_graph.figure, trim_levels_beyond=1e-3, **wavefunctions_kwargs
    )
    return w


def split_schrodinger_graph(
    schrodinger_result,
    ...
    **kwargs
    ...
    conduction_data = [
        GraphData(x, potentials["Ve"], linewidth=2, color="grey", alpha=potentialalpha)
    ]
    valence_data = [
        GraphData(
            x,
...
def split_schrodinger_graph_potentials(
    schrodinger_result,
    trim_levels_beyond=1e-2,
    linewidth=1,
    scale=0.3,
    ...
    **kwargs
    ...
    fig, (ax1, ax2) = plt.subplots(
        nrows=2, ncols=1, figsize=defaults["figsize"], dpi=defaults["dpi"]
    )
    ...
    e_data = prepare_wavefunction_data_only(
        x,
        energy_levels["Ee"] / q,
        wavefunctions["psi_e"],
        trim_levels_beyond,
    ...
    hh_data = prepare_wavefunction_data_only(
        x,
        energy_levels["Ehh"] / q,
        wavefunctions["psi_hh"],
        trim_levels_beyond,
    ...
    lh_data = prepare_wavefunction_data_only(
        x,
        energy_levels["Elh"] / q,
        wavefunctions["psi_lh"],
        trim_levels_beyond,
    ...
    if defaults["pdf"]:
        handle, path = tempfile.mkstemp(
            prefix="tmp_solcore_", suffix=".%s" % graph_defaults["format"]
        )
...
def LDOS1D_e(x, E, psi, m, step=0.001, margin=0.02, broad=0.005):
    ...
    for i, ee in enumerate(E["Ee"]):
        ...
        LDOS = LDOS + m_plane[i] / pi / hbar ** 2 * np.outer(
            L(energy, ee, broad * q), psi["psi_e"][i] ** 2
        )
...
def calculate_in_plane_masses(x, psi, m):
    """Calculates the in-plane effective mass for each level, considering that the
    wavefunction leaks into the barriers."""
...
def prepare_wavefunction_data_only(
    x,
    E,
    Psi,
    trim_levels_beyond,
