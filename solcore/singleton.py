...
class Singleton(type):  # META!
    ...
    class Redirect_to_singleton_method:
        """Class that wraps the method that has been defined as 'breakout' with its own
        class, so it can be called as a standalone function."""
    ...
    class breakoutFunctions:
        """Decorator for singletons which should make some of their methods available in
        a certain namespace.

        This is identical to the 'breakoutClass' function defined in
        'source_managed_class', but has the option of making the function available not
        in the global scope but somewhere else.
        """
        ...
        def __call__(self, cls: "Singleton") -> "Singleton":
            """And this is really the decorator for singletons which should make some of
            their methods available somewhere else."""
            if verbose:
                print(
                    "Making functions from '{}' available "
                    "in the '{}' scope: ".format(
                        cls.__name__, self.namespace_to_breakout_into.__name__
                    ),
                    end="",
                )
            ...
            for funcname in cls.__dict__:
                ...
                if hasattr(func, "break_me_out"):
                    ...
                    setattr(
                        self.namespace_to_breakout_into,
                        funcname,
...
if __name__ == "__main__":
    ...
    class ForeverAlone(metaclass=Singleton):
        ...
        @Singleton.breakout
        def rofl_this_will_never_work(self, end_symbol: str, end_number: int) -> str:
