...
def download_db(url=None, interpolation_points=200, confirm=False):
    ...
    :param url: URL from which the zip archive of the database will be downloaded.
    Default is "https://refractiveindex.info/download/database/rii-
    database-2017-09-05.zip" :param interpolation_points: how many interpolation points
...
def search_db(term="", exact=False):
    ...
    :param term: search term (e.g. the name of a material, or a source). :param exact:
    search by exact (True) or approximate (False, default) terms. :return: A list of
...
def create_nk_txt(pageid, file, folder=""):
    ...
    with the first column being the wavelength in metres and the second column the n or
    k values.

    :param pageid: pageid (number) of the database entry to be used :param file: name of
    the file to be created: the n and k data will be saved separately as [file]_n.txt
