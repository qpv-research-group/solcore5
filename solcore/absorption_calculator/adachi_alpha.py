...
from solcore.constants import c, hbar, pi
from solcore.science_tracker import science_reference


def create_adachi_alpha(material, Esteps=(1.42, 6, 3000), T=300, wl=None):
    """Calculates the n, k and absorption coefficient of a material using Adachi's
    formalism of critical points.

    :param material: A solcore material :param Esteps: (1.42, 6, 3000) A tuple with the
    start, end and step energies in which calculating the optical data :param T: (300)
    Temeprature in kelvin :param wl: (None) Optional array indicating the wavelengths in
    which calculating the data :return: A tuple containing 4 arrays: (Energy, n, k,
    alpha)
    """
    ...
    b2 = 44 * (e1 + 2 * Delta1 / 3.0) / (a0 * (e1 + Delta1) ** 2)
    ...
    f_Xi_s0 = (XiSO) ** -2 * (
        2 - (1 + XiSO) ** 0.5 - (1 - XiSO) ** 0.5 * H(1 - XiSO)
    )  #

    # equation 7
    epsilon_2_e0 = (a / E ** 2) * (
        (E - e0) ** 0.5 * H((Xi0) - 1)
        + 0.5 * (E - e0 - Delta0) ** 0.5 * H((E / (e0 + Delta0)) - 1)
    )  #
    epsilon_1_e0 = (
        a * e0 ** -1.5 * (f_Xi_0 + 0.5 * (e0 / (e0 + Delta0)) ** 1.5 * f_Xi_s0)
    )
    ...
    epsilon_2_e1 = (
        pi
        * Xi1 ** -2
        * (b1 - b11 * (e1 - E) ** 0.5 * H(1 - Xi1))
        * H(re(b1 - b11 * (e1 - E) ** 0.5 * H(1 - Xi1)))
    )  #
    epsilon_2_e1_d1 = (
        pi * Xi1s ** -2 * (b2 - b21 * (e1 + Delta1 - E) ** 0.5 * H(1 - Xi1s))
    ...
    epsilon_1_3dcp = -b1 * Xi1damped ** -2 * np.log(
        1 - Xi1damped ** 2
    ) - b2 * Xi1sdamped ** -2 * np.log(
        1 - Xi1sdamped ** 2
    )  # adachi version
    epsilon_2_3dcp = pi * b1 * Xi1damped ** -2 * H(
        Xi1 - 1
    ) + pi * b2 * Xi1sdamped ** -2 * H(
        Xi1s - 1
    )  # +10*sin(30*E)#

    # equation 16
    epsilon_2_e2 = (
        cc * Xi2 * gamma / ((1 - Xi2 ** 2) ** 2 + (Xi2 * gamma) ** 2) * H(E - e1)
    )  # added last term for compatibility with adachi's graph, dammit.
    ...
    epsilon_2_indirect = (
        d
        / E ** 2
        * (E - egid + hbar * omegaphonon) ** 2
        * H(1 - XiG_plus)
        * H(1 - XiCh)
    )  #
    epsilon_2_indirect_b = (
        d
        / E ** 2
        * (E - egid - hbar * omegaphonon) ** 2
        * H(1 - XiG_minus)
