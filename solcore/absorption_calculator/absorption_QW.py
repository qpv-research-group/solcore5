...
L = (
    lambda x, centre, hwhm: 1
    / pi
    * (0.5 * hwhm)
    / ((x - centre) ** 2 + (0.5 * hwhm) ** 2)
)  # Lorenzian (area normalised to 1)
Gauss = (
    lambda x, centre, hwhm: 1
    / np.sqrt(2 * pi)
    / (0.5 * hwhm)
    * np.exp(-0.5 * (x - centre) ** 2 / (0.5 * hwhm) ** 2)
)  # Gaussian (area normalised to 1)


def exciton_rydberg_energy_2d(me, mh, eps_r):
    """:param me: electron effective mass (units: kg)

    :param mh: hole effective mass (units: kg) :param eps_r: dielectic constant (units:
    SI) :return: The exciton Rydberg energy
    """

    science_reference(
        "Definition of the exciton Rydberg Energy for a quantum well.",
    ...
    if True:
        return (
            mr
            * q ** 4
            / (2 * hbar ** 2 * (4 * np.pi * eps_r * vacuum_permittivity) ** 2)
        )
    Ry = 13.6 * 1.6e-19
    m0 = electron_mass
...
def exciton_bohr_radius(me, mh, eps):
    ...
    :param mh: hole effective mass (units: kg) :param eps: dielectic constant (units:
    SI) :return: Exciton Borh radius
    """

    science_reference(
        "Definition of the exciton bohr radius for a quantum well.",
        "S. L. Chuang, Physics of Optoelectonic Devices, Second Edition, p.554, Table 13.1",
    )
    mr = me * mh / (me + mh)  # reduced effective mass
...
def alpha_c_hh_TE(E, z, E_e, E_hh, psi_e, psi_hh, well_width, me, mhh, Ep, nr):
    """Absortion coefficient for incident light forming a transision between hh and c
    band of a quantum well.

    NB. Assumes that valence band is a zero energy, might need to manually apply an
    offset.  :param E: photon energy (units: J) :param z: mesh points along growth
    direction (units: m) :param E_e: Electron state energy (units: J) :param E_hh: Heavy
    hole state energy (units: J) :param psi_e: Electron envelope function (psi_e^2 must
    be normalised) :param psi_hh: Heavy hole envelope function (psi_hh^2 must be
    normalised) :param well_width: (units: m) :param me: electron effective mass (units:
    kg) :param mhh: heavy hole effective mass (units: kg) :param Ep: the Kane parameter
    "Optical dipole matrix elemet", sometimes "Momentum matrix element", e.g. Ep for
    GaAs ~ 28eV :param nr: refractive index :return:
    """
...
def alpha_exciton_ehh_TE(
    ...
    :param E: :param z: :param E_e: :param E_hh: :param psi_e: :param psi_hh: :param
    well_width: :param me: :param mhh: :param Ep: :param nr: :param eps: :param hwhm:
    :param dimensionality: :param line_shape: :return:
    """
    if exciton_index < 1:
    ...
    if dimensionality < 0 or dimensionality > 0.5:
        raise ValueError(
            "The dimensionality parameter of the exciton must be a number between 0 and 0.5 (inclusive)."
        )
...
def alpha_exciton_elh_TE(
    ...
    :param E: :param z: :param E_e: :param E_lh: :param psi_e: :param psi_lh: :param
    well_width: :param me: :param mlh: :param Ep: :param nr: :param eps: :param hwhm:
    :param dimensionality: :param line_shape: :return:
    """
    if exciton_index < 1:
        raise ValueError("The excition index must start on 1.")
...
def sum_alpha_c_hh_TE(E, z, E_e, E_hh, psi_e, psi_hh, well_width, me, mh, Ep, nr):
    ...
    :param z: :param E_e: :param E_hh: :param psi_e: :param psi_hh: :param well_width:
    :param me: :param mh: :param Ep: :param nr: :return:
    """
    alpha = 0
...
def sum_alpha_c_lh_TE(E, z, E_e, E_lh, psi_e, psi_lh, well_width, me, mh, Ep, nr):
    ...
    :param z: :param E_e: :param E_lh: :param psi_e: :param psi_lh: :param well_width:
    :param me: :param mh: :param Ep: :param nr: :return:
    """
    alpha = 0
    for ee, pe in zip(E_e, psi_e):
...
def sum_alpha_exciton_c_hh_TE(
    ...
    :param z: :param E_e: :param E_hh: :param psi_e: :param psi_hh: :param well_width:
    :param me: :param mh: :param Ep: :param nr: :param eps: :param hwhm: :param
    dimensionality: :param line_shape: :return:
    """
    alpha = 0
    eh_pairs_with_zero_angular_momentum = zip(
        range(len(E_e)), range(len(E_hh))
    )  # l=0 for optically allowed transitions
    for exciton_index, eh_quantum_numbers in enumerate(
        eh_pairs_with_zero_angular_momentum
    ):
...
def sum_alpha_exciton_c_lh_TE(
    ...
    :param z: :param E_e: :param E_lh: :param psi_e: :param psi_lh: :param well_width:
    :param me: :param mlh: :param Ep: :param nr: :param eps: :param hwhm: :param
    dimensionality: :param line_shape: :return:
    """
    alpha = 0
    eh_pairs_with_zero_angular_momentum = zip(
        range(len(E_e)), range(len(E_lh))
    )  # l=0 for optically allowed transitions
    for exciton_index, eh_quantum_numbers in enumerate(
        eh_pairs_with_zero_angular_momentum
    ):
...
def calc_alpha(
    ...
    """Calculates the absorption coeficient of a quantum well structure assuming the
    parabolic approximation for the effective masses.

    :param QM_result: The output of the Schrodinger solver, incldued in the
    'quantum_mechanics' package :param well_width: The well width :param kane_parameter:
    The Kane parameter :param refractive_index: Refractive (effective) index of the QW
    :param hwhm: Full width at half maximum of the excitonic lineshape :param
    dimensionality: :param theta: :param eps: :param espace: :param line_shape: :return:
    """
    results = QM_result
    ...
    result_ex_c_lh_TE = []
    for e in espace:
        c_hh = sum_alpha_c_hh_TE(
            e,
            results["x"],
            results["E"]["Ee"],
            results["E"]["Ehh"],
            results["wavefunctions"]["psi_e"],
            results["wavefunctions"]["psi_hh"],
            well_width,
            me,
            mhh,
            kane_parameter,
        ...
        c_lh = sum_alpha_c_lh_TE(
            e,
            results["x"],
            results["E"]["Ee"],
            results["E"]["Elh"],
            results["wavefunctions"]["psi_e"],
            results["wavefunctions"]["psi_lh"],
            well_width,
            me,
            mlh,
            kane_parameter,
        ...
        ex_c_hh = sum_alpha_exciton_c_hh_TE(
            e,
            results["x"],
            results["E"]["Ee"],
            results["E"]["Ehh"],
        ...
        ex_c_lh = sum_alpha_exciton_c_lh_TE(
            e,
            results["x"],
            results["E"]["Ee"],
            results["E"]["Elh"],
    ...
    result_TE_raw = (
        result_c_hh_TE + result_c_lh_TE + result_ex_c_hh_TE + result_ex_c_lh_TE
    )
...
def NonBlackBodyEmission(E, voltage=0, nr=3.5, T=300):
    ...
    :param voltage: :param nr: :param T: :return:
    """
    A = 2 * nr ** 2 / h ** 3 / c ** 2
    B = (E - q * voltage) / kb / T
...
def calc_emission(QM_result, well_width, voltage=0, theta=0):
    ...
    :param well_width: :param voltage: :param theta: :return:
    """
    ...
    for index, e in enumerate(alfa[0]):
        ...
        TE = bb * (
            ...
            - np.exp(
                -aTE[index]
                * well_width
                * (np.cos(theta) ** 2 + 0.5 * np.sin(theta) ** 2)
            )
        )
