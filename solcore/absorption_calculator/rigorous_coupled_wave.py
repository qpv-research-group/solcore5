...
def calculate_rat_rcwa(
    structure, size, orders, wavelength, theta=0, phi=0, pol="u", substrate=None
):
    """Calculates the reflected, absorbed and transmitted intensity of the structure for
    the wavelengths and angles defined using an RCWA method implemented using the S4
    package.

    :param structure: A solcore Structure object with layers and materials or a
    OptiStack object. :param size: list with 2 entries, size of the unit cell (right
    now, can only be rectangular :param orders: number of orders to retain in the RCWA
    calculations. :param wavelength: Wavelengths (in nm) in which calculate the data.
    :param theta: polar incidence angle (in degrees) of the incident light. Default: 0
    (normal incidence) :param phi: azimuthal incidence angle in degrees. Default: 0
    :param pol: Polarisation of the light: 's', 'p' or 'u'. Default: 'u' (unpolarised).
    :param substrate: semi-infinite transmission medium :return: A dictionary with the
    R, A and T at the specified wavelengths and angle.
    """
    ...
    if pol in "sp":
        ...
        for i, wl in enumerate(
            wavelength
        ):  # set the material values and indices in here
            ...
            output["T"][i] = out["T"]
            output["A_layer"][i] = rcwa_absorption_per_layer(
                S, len(stack_OS.get_widths())
            )
...
def rcwa_rat(S, n_layers):
    ...
    R = 1 - sum(
        S.GetPowerFlux("layer_2")
    )  # GetPowerFlux gives forward & backward Poynting vector, so sum to get power flux
        ...
        if bool(geometry):
            for shape in geometry:
                ...
                if shape["type"] == "circle":
                    S.SetRegionCircle(
                        layer_name, mat_name, shape["center"], shape["radius"]
                    )
...
def update_epsilon(S, stack_OS, shape_mats_OS, wl):
    ...
    for i1 in range(
        len(shape_mats_OS.widths)
    ):  # initialise the materials needed for all the shapes in S4
        S.SetMaterial(
            "shape_mat_" + str(i1 + 1), shape_mats_OS.get_indices(wl)[i1 + 1] ** 2
        )
...
def calculate_absorption_profile_rcwa(
    ...
    not thin enough, the calculation might diverege at short wavelengths.  For now, it
    only works for normal incident, coherent light.  :param structure: A solcore
    structure with layers and materials. :param size: list with 2 entries, size of the
    unit cell (right now, can only be rectangular :param orders: number of orders to
    retain in the RCWA calculations. :param wavelength: Wavelengths (in nm) in which
    calculate the data. :param rat_output: output from calculate_rat_rcwa :param
    z_limit: Maximum value in the z direction at which to calculate depth-dependent
    absorption (nm) :param steps_size: if the dist is not specified, the step size in nm
    to use in the depth-dependent calculation :param dist: the positions (in nm) at
    which to calculate depth-dependent absorption :param theta: polar incidence angle
    (in degrees) of the incident light. Default: 0 (normal incidence) :param phi:
    azimuthal incidence angle in degrees. Default: 0 :param pol: Polarisation of the
    light: 's', 'p' or 'u'. Default: 'u' (unpolarised). :param substrate: semi-infinite
    transmission medium :return: A dictionary containing the positions (in nm) and a 2D
    array with the absorption in the structure as a function of the position and the
    wavelength.
    """
        ...
        for i, wl in enumerate(wavelength):
            ...
            for j, d in enumerate(dist):
                layer, d_in_layer = tmm.find_in_structure_with_inf(
                    stack_OS.get_widths(), d
                )  # don't need to change this
                layer_name = "layer_" + str(
                    layer + 1
                )  # layer_1 is air above so need to add 1
...
def rcwa_position_resolved(S, layer, depth, A):
    if A > 0:
        ...
        power_difference = np.real(
            sum(S.GetPowerFlux(layer, depth - delta))
            - sum(S.GetPowerFlux(layer, depth + delta))
        )
