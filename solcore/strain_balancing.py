from numpy import array, linspace, log, ones, pi, sqrt
from scipy.optimize import bisect

from solcore import ParameterSystem, asUnit, convert
...
def optimise(
    ...
    barrier_fraction,
    lattice_material,
    lattice_material_fraction=0,
    parameter_to_optimise="well_thickness",
    T=300,
):
    """This can't handle sol-core materials yet.
    ...
    if parameter_to_optimise not in options:
        print(
            "ERROR: Wring option in strain_balancing.optimise. "
    ...
    a0 = get_vurgaftman_parameter(
        lattice_material, "lattice_constant", x=lattice_material_fraction, T=T
    )
    ...
    if parameter_to_optimise == "barrier_fraction":
        ...
        def func(x):
            ...
            A2 = C11_barrier + C12_barrier - 2 * C12_barrier ** 2 / C11_barrier
            a2 = get_vurgaftman_parameter(
                barrier_material, "lattice_constant", x=x, T=T
            )
            a2 = convert(a2, "angstrom", "m")

            if a0 == a2 and x == 0:
                a2 = get_vurgaftman_parameter(
                    barrier_material, "lattice_constant", x=0.001, T=T
                )
                a2 = convert(a2, "angstrom", "m")
...
def critical_thickness(
    ...
    lattice_material="GaAs",
    layer_fraction=0,
    lattice_fraction=None,
    T=300,
    during_growth=True,
    final_unit="m",
    bowed_material="In",
    ...
    other_params = {"T": T, bowed_material: layer_fraction}
    ...
    a_lattice = get_vurgaftman_parameter(
        lattice_material, "lattice_constant", **other_params
    )
    ...
    for vi, fi, bi in zip(v, f, b):
        roots_at_soln = lambda hc_over_b: (hc_over_b) / (log(hc_over_b) + 1) - (
            1 - vi / 4
        ) / (
            fi * pi * (1 + vi)
        )  # matthews & blakeslee
        ...
        result.append(
            convert(hc / 4, "Ang", final_unit)
        ) if during_growth else result.append(convert(hc, "Ang", final_unit))
    return array(result)
...
if __name__ == "__main__":
    ...
    g = Graph(
        GraphData(
            T, asUnit(result, "nm"), label="Matthews-Blakeslee $\\frac{h_c}{4}$"
        ),  # , "-", 1,"red"),
        # yscale="log",
        # xlim=(0.03,0.53),
        # ylim=(1,1000),
