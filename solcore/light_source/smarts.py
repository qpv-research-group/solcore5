...
smarts = solcore.config.get("External programs", "smarts").replace(
    "SOLCORE_ROOT", solcore.SOLCORE_ROOT
)
...
def calculate_spectrum_smarts(
    smarts_file_contents=None, filename="smarts295", target_directory=None
):
    """:param smarts_file_contents:

    :param filename: :param target_directory: :return:
    """
    ...
    if target_directory is not None:
        ...
        assert os.access(target_directory, os.W_OK), (
            "ERROR: Target folder for smarts output does not exists "
            "or is not writable."
        )
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            if os.path.exists(scn_file):
                ...
                if target_directory is not None:
                    shutil.copy2(
                        scn_file, scn_file.replace(working_directory, target_directory)
                    )
            if os.path.exists(inp_file):
                if target_directory is not None:
                    shutil.copy2(
                        inp_file, inp_file.replace(working_directory, target_directory)
                    )
            if os.path.exists(ext_file):
                if target_directory is not None:
                    shutil.copy2(
                        ext_file, ext_file.replace(working_directory, target_directory)
                    )
            if os.path.exists(out_file):
                if target_directory is not None:
                    shutil.copy2(
                        out_file, out_file.replace(working_directory, target_directory)
                    )
    ...
    if len(data) == 0:
        raise ValueError(
            "ERROR in SMARTS: Output file is empty. Likely error in the input parameters."
        )
...
def build_smarts_file(**kwargs):
    try:
        ...
        smarts_file_contents.append(
            ["{TAIR} {RH} '{SEASON}' {TDAY} !Card 3a", "'{ATMOS}' !Card 3a"][
                kwargs["IATMOS"]
            ]
        )
        ...
        print(
            "The SMARTS options you have selected require additional data, variables are undefined."
        )
        print(err)
        raise
...
def get_default_smarts_object():
    """Provides a dictionary with most of the parameters (values of the CARDS) needed by
    SMARTS to calculate a solar spectrum. It can be used as a template to customize for
    ...
    preasure_model = (
        1
    )  # 'ISPR', surface preasure model set to real data + altitude correction
    ...
    solar_position_mode = (
        3
    )  # 'IMASS', use the location and time to calculate the position of the sun

    # atmospheric conditions
    atmospheric_data = 1  # 'IATMOS', allows to input the correct atmospheric data
    atmosphere_model = (
        "USSA"
    )  # 'ATMOS', 'US standard spectrum', general atmospheric model, setting all parameters not given as input
    ...
    which_variables = (
        "4"
    )  # 'IOUT', code of the output variables. Set to all tilted irradiances + experimental with FoV
    ...
    sun_correction = (
        1
    )  # 'SUNCOR', distance to the Sun correction faction. It is not used
    ...
    comment = (
        "Test"
    )  # 'COMNT', single line with a comment on the run. Max 64 characters, no spaces but underscores
    ...
    targetTime = datetime(
        2015, 5, 19, 12, 30
    )  # has to be split in 'YEAR', 'MONTH', 'DAY', 'HOUR', the later in Local Standard Time
