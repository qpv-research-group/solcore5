...
n_spline = InterpolatedUnivariateSpline(x=Ge_nk_Exp[::5, 0], y=Ge_nk_Exp[::5, 1], k=3)(
    E_eV
)
k_spline = InterpolatedUnivariateSpline(x=Ge_nk_Exp[::5, 2], y=Ge_nk_Exp[::5, 3], k=3)(
    E_eV
)
...
eps = lambda r, theta: np.sin(theta) ** 2 * (
    1 + np.tan(theta) ** 2 * ((1 - r) / (1 + r)) ** 2
)

# Experimental data...
Exp_rho = rho(
    np.radians(Exp_Data.data[Exp_Angles[i]][1]),
    np.radians((Exp_Data.data[Exp_Angles[i]][3])),
)
...
# Experimental data...
ax1.plot(
    Exp_Data.data[Exp_Angles[i]][0] * 1000,
    Exp_eps.real,
    lw=2,
    marker="o",
    ls="none",
...
ax1.plot(
    1240 / E_eV,
    Mod_eps.real,
    label="Model $\epsilon_1 (\lambda)$ :: $ %3.1f^{\circ}$" % Exp_Angles[i],
    color=colours("Maroon"),
)
ax1b.plot(
    1240 / E_eV,
    abs(Mod_eps.imag),
    label="Model $\epsilon_2 (\lambda)$ :: $ %3.1f^{\circ}$" % Exp_Angles[i],
    color=colours("Navy"),
)
