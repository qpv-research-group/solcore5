...
for k, rad in enumerate([False, True]):
    ...
    db_junction = Junction(
        kind="2D", T=T, reff=0.3, jref=300, Eg=0.66, A=1, R_shunt=np.inf, n=3.5
    )
    db_junction2 = Junction(
        kind="2D", T=T, reff=1, jref=300, Eg=1.4, A=1, R_shunt=np.inf, n=3.5
    )
    db_junction3 = Junction(
        kind="2D", T=T, reff=1, jref=300, Eg=1.8, A=1, R_shunt=np.inf, n=3.5
    )

    my_solar_cell = SolarCell(
        [db_junction3, db_junction2, db_junction], T=T, R_series=0
    )
