...
from optimization_functions import PDE, DE
import os
...
#
#
# # Now that best total layer thicknesses for
#
# def this_dir_file(f):
#     return os.path.join(os.path.split(__file__)[0], f)
#
# starting_params = np.append(best_pop, [30000, 25])
# n_type_thickness = np.append(0.2*starting_params[2:5], 1000)
#
# lower = 0.75*starting_params
# upper = 1.25*starting_params
#
# lower_ntype = [5, 5, 5, 100]
#
# upper_ntype = np.append(upper[2:5]/2, 3000)
#
# all_starting = np.append(starting_params, n_type_thickness)
#
# all_lower = np.append(lower, lower_ntype)
#
# all_upper = np.append(upper, upper_ntype)
#
# all_bounds = np.stack((all_upper, all_lower)).T
#
# print(all_lower)
# print(all_upper)
#
...
#         #x[1]: Ta2O5 thickness
#
#         #x[2]: total InGaP thickness
#         #x[3]: total InGaAs thickness
#         #x[4]: total SiGeSn thickness
#         #x[5]: total Ge thickness
#
#         #x[6]: window layer thickness
#
#         #x[7]: InGaP n thickness
#         #x[8]: InGaAs n thickness
#         #x[9]: SiGeSn n thickness
#         #x[10]: Ge n thickness
#
#         e_charge = si('1eV')
#
...
#
#
#         Ge_nk = np.loadtxt(this_dir_file("ge_n_k_genosc.txt"), unpack=True, delimiter="\t")
#         Ge.n = interp1d(x=Ge_nk[0] / 1e9, y=Ge_nk[1])
#         Ge.k = interp1d(x=Ge_nk[0] / 1e9, y=Ge_nk[2])
#         Ge.alpha = interp1d(x=Ge_nk[0] / 1e9, y=4 * np.pi * Ge_nk[2] / (Ge_nk[0] / 1e9))
#
#         # Ge_mobility_h = 0.38 #
#         Ge_lifetime_h = 1e-6
...
#             Layer(si(x[0], 'nm'), material=MgF2), Layer(si(x[1], 'nm'), material=Ta2O5),
#             Junction([Layer(si(x[6], 'nm'), material=window_material, role='window'),
#                       Layer(si(x[7], 'nm'), material=top_cell_n_material, role='emitter'),
#                       Layer(si(x[2]-x[7], 'nm'), material=top_cell_p_material, role='base'),
#                       ], sn=1, sp=1, kind='DA'),
#             Junction([Layer(si(x[7], 'nm'), material=mid_cell_n_material, role='emitter'),
#                       Layer(si(x[3]-x[8], 'nm'), material=mid_cell_p_material, role='base'),
#                       ], sn=1, sp=1, kind='DA'),
#             Junction([Layer(si(x[9], 'nm'), material=pen_cell_n_material, role='emitter'),
#                       Layer(si(x[4]-x[9], 'nm'), material=pen_cell_p_material, role='base'),
#                       ], sn=1, sp=1, kind='DA'),
#             Junction([Layer(si(x[10], 'nm'), material=bot_cell_n_material, role='emitter'),
#                       Layer(si(x[5]-x[10], 'nm'), material=bot_cell_p_material, role='base'),
#                       ], sn=1, sp=1, kind='DA'),
...
#         # # plt.plot(V, -solar_cell[5].iv(V)/1e4, 'y', label='Ge')
#         # plt.text(2, 10, '$\eta = $' + str(round(efficiency * 100, 1)) + '%')
#         # # plt.text(1,8.5*concX/500,'Pmax='+str(round(pmax/1e4,1))+'W/cm$^2$')
#         # plt.text(2, 9, 'FF = ' + str(round(ff * 100, 1)) + '%')
#         # # plt.text(1,7.5*concX/500,'Voc='+str(round(voc,1))+'V')
#         # # plt.text(1,7*concX/500,'Jsc='+str(round(isc/1e4,1))+'A/cm$^2$')
#         #
#         # plt.legend()
#         # plt.ylim(0, 18)
#         # plt.xlim(0, 3.5)
...
#                           user_options={'wavelength': wl, 'optics_method': 'TMM', 'BL_correction': True})
#
#         # plt.figure()
