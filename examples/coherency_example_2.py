...
def this_dir_file(
    f
):  # this is just Python stuff, so it can find external files containing optical constant data which are in the same folder as this file.
...
light_source = LightSource(
    ...
    concentration=concX,
)  # define the input light source as AM1.5G
...
top_cell_n_material = GaInP(
    In=0.51, Nd=siUnits(2e18, "cm-3"), hole_diffusion_length=si("300nm")
)
top_cell_p_material = GaInP(
    In=0.51, Na=siUnits(1.5e17, "cm-3"), electron_diffusion_length=si("2um")
)
...
mid_cell_p_material = GaAs(
    Na=siUnits(1.5e17, "cm-3"), electron_diffusion_length=si("3um")
)
...
DBRa = 16 * [
    Layer(width=si("62.638nm"), material=DBR1),
    Layer(width=si("71.980nm"), material=DBR2),
]
DBRb = 16 * [
    Layer(width=si("68.919nm"), material=DBR1),
    Layer(width=si("78.725nm"), material=DBR2),
]
DBRc = 16 * [
    Layer(width=si("75.838nm"), material=DBR1),
    Layer(width=si("86.805nm"), material=DBR2),
]
...
optical_struct = SolarCell(
    ARC + top_junction + middle_junction + DBRa + DBRb + DBRc + bottom_junction,
    shading=0.05,
)
...
plt.figure()
plt.plot(
    wl * 1e9, optical_struct[0].layer_absorption + optical_struct[1].layer_absorption
)
