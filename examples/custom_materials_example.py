...
from solcore.material_data.refractiveindex_info import (
    create_nk_txt,
    download_db,
    search_db,
)
...
create_new_material(
    "SiGeSn", "data/SiGeSn_n.txt", "data/SiGeSn_k.txt", "data/SiGeSn_params.txt"
)
...
plt.figure()
plt.plot(
    si(np.arange(300, 1700, 5), "nm") * 1e9, SiGeSn.n(si(np.arange(300, 1700, 5), "nm"))
)
plt.plot(
    si(np.arange(300, 1700, 5), "nm") * 1e9, SiGeSn.k(si(np.arange(300, 1700, 5), "nm"))
)
...
create_nk_txt(pageid=results.iloc[0].pageid, file="C_Diamond")
create_new_material(
    mat_name="Diamond", n_source="C_Diamond_n.txt", k_source="C_Diamond_k.txt"
)
...
plt.figure()
plt.plot(
    si(np.arange(100, 800, 5), "nm") * 1e9, Diamond.n(si(np.arange(100, 800, 5), "nm"))
)
plt.plot(
    si(np.arange(100, 800, 5), "nm") * 1e9, Diamond.k(si(np.arange(100, 800, 5), "nm"))
)
