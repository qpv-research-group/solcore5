"""Reading in material parameters and using the modified critical point parabolic band
model (CPPB) to approximate the complex dielectric function of GaAs. This model is based
on:

- Adachi, S., Physical Properties of III-V Semiconductor Compounds, John Wiley & Sons
(1992) - C. C. Kim et al, 'Modelling the optical dielectric function of semiconductors:
Extension of the critical-point parabolic band approximation', Physical Review B 45(20)
11749, 1992
"""
...
ax1.plot(
    Palik_Eps1[:, 0],
    Palik_Eps1[:, 1],
    label="Exp. Data (Palik)",
    marker="o",
    ls="none",
    markerfacecolor="none",
    markeredgecolor=colours("Red"),
)

ax1.plot(E, Output["eps"].real, color=colours("Navy"), label="Total")
ax1.plot(
    E,
    Output["components"][0].real,
    color=colours("Orange Red"),
    ls="--",
    label="$E_0$ and $E_0+\Delta_0$",
)
ax1.plot(
    E,
    Output["components"][1].real,
    color=colours("Dodger Blue"),
    ls="--",
    label="$E_1$ and $E_1+\Delta_1$",
)
ax1.plot(
    E,
    Output["components"][2].real,
    color=colours("lime green"),
    ls="--",
    label="$E_{ID}$ (Indirect)",
)
